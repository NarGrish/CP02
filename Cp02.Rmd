...
\begin{align}
  \Delta M (f(x+h)-f(x) &= \sqrt{f(x+h)^2\sigma^2+f(x)^2\delta_M^2}) \\
  &= \sqrt{\sigma^2+\delta_M^2}|f(x)|\\ % Nicht ungefähr? ist ja nicht wirklich gleich...
  &= \sqrt{\frac{1}{3}\delta_M^2+\delta_M^2}|f(x)| = \sqrt{\frac{4}{3}}\delta_M |f(x)|
\end{align}

### Maschinenfehler der 3-Punktformel

\begin{align}
  \Delta M (f(x+h)-f(x-h) &= \sqrt{f(x+h)^2\sigma^2+f(x-h)^2\sigma^2}) \\
  &= \sqrt{2\sigma^2f(x)^2}\\
